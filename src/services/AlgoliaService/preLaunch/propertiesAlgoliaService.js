import { clientPropeties } from "..";

// Initialize Algolia search client
const searchClient = clientPropeties;

const INDEX_NAME = "truEstatePreLaunch";
...
const buildFilterString = (filters = {}) => {
  ...
  if (filters.value?.length && !filters.value.includes("all")) {
    filterParts.push(
      `(${filters.value
        .map((value) => `investmentOverview.value:"${value}"`)
        .join(" OR ")})`
    );
  }
  ...
  if (filters.status?.length && !filters.status.includes("all")) {
    filterParts.push(
      `(${filters.status
        .map((value) => `projectOverview.stage:"${value}"`)
        .join(" OR ")})`
    );
  }
